...
private:
    ...
    struct StageRobot
    {
        ...
        ros::ServiceServer cmdvel_sub; //one cmd_vel subscriber
    };
...
public:
    ...
    bool cmdvelReceived(int idx, stage_controllers::CallStageRos::Request  &req,stage_controllers::CallStageRos::Response &res);
    ...
    //For communicating with robot_nav node
    bool CallToStage(stage_controllers::CallStageRos::Request  &req,stage_controllers::CallStageRos::Response &res);
};
...
bool
StageNode::cmdvelReceived(int idx, stage_controllers::CallStageRos::Request  &req,stage_controllers::CallStageRos::Response &res)
{
    boost::mutex::scoped_lock lock(msg_lock);
    this->positionmodels[idx]->SetSpeed(req.linearX,
                                        req.linearY,
                                        req.angularZ);
...
StageNode::SubscribeModels()
{
	...
	// TODO - print the topic names nicely as well
        ...
        new_robot->cmdvel_sub = n_.advertiseService<stage_controllers::CallStageRos::Request, stage_controllers::CallStageRos::Response>(mapName(CMD_VEL, r, static_cast<Stg::Model*>(new_robot->positionmodel)), boost::bind(&StageNode::cmdvelReceived, this, r, _1, _2));

        for (size_t s = 0;  s < new_robot->lasermodels.size(); ++s)
        {
            if (new_robot->lasermodels.size() == 1)
                new_robot->laser_pubs.push_back(n_.advertise<sensor_msgs::LaserScan>(mapName(BASE_SCAN, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 0));
            else
...
std::vector<std::string> StageNode::getNames()
{
    std::vector<std::string> robotNm;
    for(int i=0;i<this->positionmodels.size();i++)
    {
       robotNm.push_back(this->positionmodels[i]->Token());
    }
    return robotNm;
}

void StageNode::thread()
{
    ros::NodeHandle n;
    Robot_nav rn;
    boost::thread t2[this->positionmodels.size()];
    for(int i=0;i<this->positionmodels.size();i++)
    {
        std::string s=this->positionmodels[i]->Token();
        t2[i]=boost::thread(boost::bind(&Robot_nav::run,&rn,&n,"/"+s+"/"));
        //Robot_nav rn(&n,"/"+s+"/");
    }
}

bool StageNode::CallToStage(stage_controllers::CallStageRos::Request  &req,stage_controllers::CallStageRos::Response &res)
{
    if(req.functionName=="rotateRobot"){
    for(int i=0;i<this->positionmodels.size();i++)
    {
        std::string s=this->positionmodels[i]->Token();
        if("/"+s+"/"==req.robotName)
        {
            boost::mutex::scoped_lock lock(msg_lock);
            Stg::Pose p=this->positionmodels[i]->GetPose();
            p.a=p.a+req.angularZ;
            this->positionmodels[i]->SetPose(p);
            this->base_last_cmd = this->sim_time;
            res.result="Success!!!";
            break;
        }
    ...
    else if(req.functionName=="moveRobot"){
    for(int i=0;i<this->positionmodels.size();i++)
    {
        std::string s=this->positionmodels[i]->Token();
        if("/"+s+"/"==req.robotName)
...
main(int argc, char** argv)
{
    ...
    ros::init(argc, argv, "stage_controllers");
    ros::NodeHandle n;
    ...
    ros::ServiceServer service = n.advertiseService<stage_controllers::CallStageRos::Request, stage_controllers::CallStageRos::Response>("CallToStage", boost::bind(&StageNode::CallToStage, &sn, _1, _2));

    boost::thread t = boost::thread(boost::bind(&ros::spin));

    sn.world->Start();
    
    //--------------------- To run robot_nav for each robots
/*
    std::string rosCmd;
    std::vector<std::string> robotNames=sn.getNames();
    for(int i=0;i<robotNames.size();i++)
    {
        rosCmd="rosrun stage_controllers robot_nav /"+robotNames[i]+"/ &";
        system(rosCmd.c_str());
   }    
*/

    //---------------------- Multi-threading
    //boost::thread t1 = boost::thread(boost::bind(&StageNode::thread,&sn));
